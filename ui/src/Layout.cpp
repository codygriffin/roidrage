//------------------------------------------------------------------------------
//
// Copyright (C) 2013 Cody Griffin (cody.m.griffin@gmail.com)
// 
//------------------------------------------------------------------------------

#include <Layout.h>
#include <Log.h>

using namespace pronghorn;
using namespace roidrage;

//------------------------------------------------------------------------------

Layout::Layout() {
  // Default
}

Layout::Layout(glm::vec2 translation, glm::vec2 scale) {
  translation_ = translation;
  scale_       = scale; 
}

Layout::Layout(unsigned width, unsigned height) {
  dimension_   = glm::vec2((float)width, (float)height);
}

void
Layout::layout() {
  layout(0, glm::vec2(0.0f, 0.0f));
}

glm::vec2
Layout::layout(Layout* pParent, glm::vec2 position) {
  position_  = position;
  dimension_ = onLayout(pParent, position_);

  Log::info("Layout pos=(%,%) dim=(%,%)", position_.x, position_.y, dimension_.x, dimension_.y);

  float y = 0.0f;
  for (auto child = children_.begin(); child != children_.end(); child++) {
    auto dim = (*child)->layout(this, glm::vec2(position_.x + 0.0f, position_.y + y));
    y += dim.y;
  }

  return dimension_;
}

glm::vec2
Layout::onLayout(Layout* pParent, glm::vec2 trans) {
  return dimension_;
}


void 
Layout::add(Layout* pLayout) {
  children_.push_back(std::unique_ptr<Layout>(pLayout));
}

void 
Layout::clear() {
  children_.clear();
}

void 
Layout::render() {
  onRender(); // Render before children

  for (auto child = children_.begin(); child != children_.end(); child++) {
    (*child)->render();
  }
}

bool 
Layout::touch(const Touch& touch) {
  const float x = touch.xs[touch.index];
  const float y = touch.ys[touch.index];

  // We handle it ourselves
  if (onTouch(touch)) return true;

  // We delegate to our children
  for (auto child = children_.begin(); child != children_.end(); child++) {
    if ((*child)->isHit(x, y)) {
      if ((*child)->touch(touch)) 
        return true;
    }
  }

  // We dropped the ball...
  return false;
}

bool 
Layout::isHit(float x, float y) {
  return x >= position_.x && x <= position_.x+dimension_.x &&
         y >= position_.y && y <= position_.y+dimension_.y;
}

//------------------------------------------------------------------------------

glm::vec2 
Layout::dimension() const {
  return dimension_;
}

//------------------------------------------------------------------------------

glm::vec2 
Layout::position() const {
  return position_;
}

//------------------------------------------------------------------------------
