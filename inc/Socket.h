...
struct Socket : public FileDescriptor {
  ...
  Socket() 
  : FileDescriptor(socket(Domain::domain, Type::type | SOCK_NONBLOCK, Protocol::proto)) {
    if (*this < 0) {
      SOCKET_THROW;
    }
  }
  ...
  recv(Buffer::iterator begin,  
       Buffer::iterator end, 
       Addr<Domain>& addr) {
    ssize_t bytesRcvd = ::recvfrom(*this, &(*begin), end-begin, 0,
                                   reinterpret_cast<sockaddr*>(&addr.addr), &addr.length);
    bool again = errno == EAGAIN;
  ...
  ssize_t recv(std::string& buffer, Addr<Domain>& addr) {
    ssize_t bytesRcvd = ::recvfrom(*this, &buffer[0], buffer.length(), 0, 
                                   reinterpret_cast<sockaddr*>(&addr.addr), &addr.length);
    if (bytesRcvd < 0 && errno != EAGAIN) {
      SOCKET_THROW;
    }
